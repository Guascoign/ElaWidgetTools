/********************************************************************************
    * 文件名称 ：Prt_DIG_Test.cpp
    * 作     者：ZM
    * 版     本：V1.0
    * 编写日期 ：2025-06-06
    * 功     能：通信协议 界面
*********************************************************************************/
#include "Prt_DIG_Test.h"
#include <algorithm>
#include <limits>

Prt_DIG_Test::Prt_DIG_Test(QWidget *parent)
    : ProtocolBaseWidget(parent)
{
    ...
    ElaText* Text1 = new ElaText("位置数据", leftTopArea);
    Text1->setFont(labelFont);
    ElaText* Text2 = new ElaText("控制数据", leftTopArea);
    Text2->setFont(labelFont);

    ElaText* x1Label = new ElaText("反馈:", leftTopArea);
    ElaText* y1Label = new ElaText("期望:", leftTopArea);
    ElaText* x1ZeroLabel = new ElaText("误差:", leftTopArea);
    ElaText* y1ZeroLabel = new ElaText("控制:", leftTopArea);
    ...
    // 记录当前模式
    connect(Mode_Select, QOverload<int>::of(&ElaComboBox::currentIndexChanged), this, &Prt_DIG_Test::onModeChanged);
    ...
    connect(SendParamBtn, &ElaPushButton::clicked, this, &Prt_DIG_Test::onSendParamClicked);

    // 创建UI更新定时器 - 优化更新频率以提高性能和流畅度
    uiUpdateTimer = new QTimer(this);
    uiUpdateTimer->setInterval(50); // 50ms = 20Hz，平衡性能和响应性
    connect(uiUpdateTimer, &QTimer::timeout, this, &Prt_DIG_Test::updateUI);
    uiUpdateTimer->start();
    ...
    customPlot->graph(0)->setName("位置");
    
    customPlot->addGraph(); // Y1曲线
    customPlot->graph(1)->setPen(QPen(QColor(255, 110, 40), 2));
    customPlot->graph(1)->setName("目标");
    
    customPlot->addGraph(); // X-DA
    customPlot->graph(2)->setPen(QPen(QColor(0, 180, 0), 2));
    customPlot->graph(2)->setName("控制");

    customPlot->addGraph(); // Y-DA
    customPlot->graph(3)->setPen(QPen(QColor(180, 0, 180), 2));
    customPlot->graph(3)->setName("误差");
...
void Prt_DIG_Test::SerialConnected(bool connected)
{
    // 连接状态处理
    if (connected) {
        // 连接成功时的逻辑
...
void Prt_DIG_Test::Creat_data(uint8_t *data, uint8_t mode, int16_t X1, int16_t Y1 )
{
    data[0] = 0xAB; // 帧头
    data[1] = mode; // 模式
    data[2] = X1 >> 8;
...
void Prt_DIG_Test::Proctol_Analysis(const QByteArray &data)
{
    recvCount++;
    if (data.size() < 12) return;

    const uint8_t *buf = reinterpret_cast<const uint8_t*>(data.constData());
    if (buf[0] != 0xAB || buf[11] != 0xCC) {
        checkErrCount++;
        return;
    }
    // 校验
    ...
    if (static_cast<uint8_t>(checksum) != buf[10]) {
        checkErrCount++;
        return;
    }

    uint8_t mode = buf[1];
    int16_t X_cur = (buf[2] << 8) | buf[3];     //x轴反馈位置
    int16_t Y_cur = (buf[4] << 8) | buf[5];     //x轴目标位置
    int16_t X_DActrl  = (buf[6] << 8) | buf[7]; //x轴DA控制 0.0001
    int16_t Y_DActrl  = (buf[8] << 8) | buf[9]; //x轴误差

    // 更新数据
    {
        QMutexLocker locker(&dataMutex);
        latestValidData.X1 = X_cur;
        latestValidData.Y1 = Y_cur;
        latestValidData.X1_Zero = Y_DActrl;  // Y_DActrl填入"误差"显示
        latestValidData.Y1_Zero = X_DActrl;  // X_DActrl填入"控制"显示
        ...
        x1Data.append(X_cur);
        y1Data.append(Y_cur);
        x1DAData.append(X_DActrl);  // 控制值
        y1DAData.append(Y_DActrl);  // 误差值

        while (timeData.size() > maxDataPoints) {
            timeData.removeFirst();
            x1Data.removeFirst();
            y1Data.removeFirst();
            x1DAData.removeFirst();
            y1DAData.removeFirst();
        }
    }
}

void Prt_DIG_Test::SerialDataReceived(QByteArray data)
{
    ...
    while (dataBuffer.size() >= 12) {
        ...
        if (dataBuffer.size() >= 12) {
            QByteArray packet = dataBuffer.left(12);
            dataBuffer.remove(0, 12);
...
void Prt_DIG_Test::COM_Send_data(uint8_t *data, uint8_t len)
{
    // 发送数据的逻辑
...
// 添加槽函数实现
void Prt_DIG_Test::onModeChanged(int index)
{
    // 保存上一个模式的参数
    switch(currentMode) {
    case 1: // 零位设置
...
void Prt_DIG_Test::onSendParamClicked()
{
    // 根据当前模式执行不同操作
    switch(currentMode) {
    case 0: // 10零位设置
        // 读取输入框参数并设置零位
        {
            ...
            uint8_t data[12];
            Creat_data(data, 0x00, X1_Zero, Y1_Zero);
...
void Prt_DIG_Test::updateUI()
{
    ...
    if (latestValidData.hasNewData) {
        ...
        if (X1_Zero) X1_Zero->setText(QString::number(latestValidData.X1_Zero)); // 显示Y_DActrl(误差)
        if (Y1_Zero) Y1_Zero->setText(QString::number(latestValidData.Y1_Zero)); // 显示X_DActrl(控制)
        ...
        if (customPlot && !timeData.isEmpty()) {
            ...
            customPlot->graph(0)->setData(timeData, x1Data);
            customPlot->graph(1)->setData(timeData, y1Data);
            customPlot->graph(2)->setData(timeData, x1DAData);
            customPlot->graph(3)->setData(timeData, y1DAData);
...
void Prt_DIG_Test::processBufferedData()
{
    // 预留函数，用于处理更复杂的数据缓冲逻辑
