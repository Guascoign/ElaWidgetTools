/********************************************************************************
    * 文件名称 ：Prt_DIG45_Test.cpp
    * 作     者：ZM
    * 版     本：V1.0
    * 编写日期 ：2025-06-06
    * 功     能：通信协议 界面
*********************************************************************************/
#include "Prt_DIG45_Test.h"
#include <algorithm>
#include <limits>

Prt_DIG45_Test::Prt_DIG45_Test(QWidget *parent)
    : ProtocolBaseWidget(parent)
{
    ...
    ElaText* x1fedb = new ElaText("反馈:", leftTopArea);
    ...
    // 记录当前模式
    connect(Mode_Select, QOverload<int>::of(&ElaComboBox::currentIndexChanged), this, &Prt_DIG45_Test::onModeChanged);
    ...
    connect(SendParamBtn, &ElaPushButton::clicked, this, &Prt_DIG45_Test::onSendParamClicked);

    // 创建UI更新定时器 - 优化更新频率以提高性能和流畅度
    uiUpdateTimer = new QTimer(this);
    uiUpdateTimer->setInterval(50); // 50ms = 20Hz，平衡性能和响应性
    connect(uiUpdateTimer, &QTimer::timeout, this, &Prt_DIG45_Test::updateUI);
    uiUpdateTimer->start();
...
void Prt_DIG45_Test::SerialConnected(bool connected)
{
    // 连接状态处理
    if (connected) {
        // 连接成功时的逻辑
...
void Prt_DIG45_Test::Creat_data(uint8_t *data, uint8_t mode, int16_t X1, int16_t Y1 )
{
    data[0] = 0xAB; // 帧头
    data[1] = mode; // 模式
    data[2] = X1 >> 8;
...
void Prt_DIG45_Test::Proctol_Analysis(const QByteArray &data)
{
    recvCount++;
...
void Prt_DIG45_Test::SerialDataReceived(QByteArray data)
{
...
void Prt_DIG45_Test::COM_Send_data(uint8_t *data, uint8_t len)
{
    // 发送数据的逻辑
...
// 添加槽函数实现
void Prt_DIG45_Test::onModeChanged(int index)
{
    // 保存上一个模式的参数
    switch(currentMode) {
    case 1: // 零位设置
...
void Prt_DIG45_Test::onSendParamClicked()
{
    // 根据当前模式执行不同操作
    switch(currentMode) {
    case 0: // 10零位设置
...
void Prt_DIG45_Test::processBufferedData()
{
    // 预留函数，用于处理更复杂的数据缓冲逻辑
